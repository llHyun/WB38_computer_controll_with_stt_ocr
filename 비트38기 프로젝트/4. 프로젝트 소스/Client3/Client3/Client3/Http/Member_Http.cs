        ...
        public async Task<bool> Signup(string accinfo)
        {
            try
            {
                ...
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    // 서버로 POST 요청
                    HttpResponseMessage response = await httpClient.PostAsync("http://192.168.17.192:3000/LoginRouter/signup", content);
                    //response값 확인
        ...
        public async Task<(bool, string)> auto(string token)
        {
            try
            {
                ...
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    // 서버로 POST 요청
                    HttpResponseMessage response = await httpClient.PostAsync("http://192.168.17.192:3000/LoginRouter/autologin", content);
                    //response값 확인
                    if (response.IsSuccessStatusCode)
                    {
                        string responseText = await response.Content.ReadAsStringAsync();
        ...
        public async Task<(bool, string)> Login(string set)
        {
            try
            {
                ...
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    // 서버로 POST 요청
                    HttpResponseMessage response = await httpClient.PostAsync("http://192.168.17.192:3000/LoginRouter/login", content);
                    //response값 확인
                    if (response.IsSuccessStatusCode)
                    {
                        string responseText = await response.Content.ReadAsStringAsync();
        ...
        public async Task<bool> Withdraw(string accinfo)
        {
            try
            {
                ...
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    // 서버로 POST 요청
                    HttpResponseMessage response = await httpClient.PostAsync("http://192.168.17.192:3000/UserMenuRouter/withdraw", content);
                    //response값 확인
                    if (response.IsSuccessStatusCode)
                    {
                        string responseText = await response.Content.ReadAsStringAsync();
        ...
        public async Task<bool> Update(string accinfo)
        {
            try
            {
                ...
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    // 서버로 POST 요청
                    HttpResponseMessage response = await httpClient.PostAsync("http://192.168.17.192:3000/UserMenuRouter/user_update", content);
                    //response값 확인
                    if (response.IsSuccessStatusCode)
                    {
                        string responseText = await response.Content.ReadAsStringAsync();
